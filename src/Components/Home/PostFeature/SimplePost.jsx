...
const SimplePost = ({post,openCommentMadal,handleReact,comments,reacts,getComments,getReacts}) => {
    
    const [lookingComments,setLookingComments] = useState(false)
    console.log(lookingComments)
    ...
    return (
        <div className="max-w-sm mt-2 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
               ...
               <div className='flex cursor-pointer' onClick={()=>openCommentMadal(post.id)}> <AiFillMessage className='mt-1' /> <span className='ml-1'>Comment</span></div>
              </div>
               </div>
...

              {
                ...
                {
                  reversedComments.map(comment => <CommentList key={comment.id} comment={comment} getComments={getComments} post={post} />)
                }
               </div>) : ''
              }
            </div>
